...
export async function syncLiveMatchesIfNecessary() {
  ...
  try {
    ...
    for (const event of relevant) {
      ...
      if (frozen && frozen.homeScore90 === null && (event.status === "extra_time" || event.status === "extratime" || event.status === "penalty" || event.status === "penalties" || event.status === "penalty_shootout")) {
      ...
      if (status === "finished" && currentScoreHome !== null && currentScoreAway !== null) {
        ...
        if (winnerTeam || decidedBy === "penalties") {
          if (decidedBy === "penalties") {
            ...
            try {
              const detail = await fetchMatchDetail(event.id) as any;
